...
static void send_status_line(void) {
    ...
    snprintf(
        ...
        last_y,
        pressed(PIN_MENU_UP),
        pressed(PIN_MENU_DOWN),
        pressed(PIN_MENU_LEFT),
        pressed(PIN_MENU_RIGHT),
        pressed(PIN_BOMB_SELECT),
        pressed(PIN_TRIGGER),
        pressed(PIN_RELOAD),
        pressed(PIN_COIN),
        pressed(PIN_START),
        pressed(PIN_TEST_CAL),
        pressed(PIN_ACTIVE),
        cfg.filter
    );
...
static void hid_keyboard_send(void) {
    ...
    if (pressed(PIN_MENU_UP) && n < 6) {
        keycode[n++] = HID_KEY_ARROW_UP;
    }

    if (pressed(PIN_MENU_DOWN) && n < 6) {
        keycode[n++] = HID_KEY_ARROW_DOWN;
    }

    if (pressed(PIN_MENU_LEFT) && n < 6) {
        keycode[n++] = HID_KEY_ARROW_LEFT;
    }

    if (pressed(PIN_MENU_RIGHT) && n < 6) {
        keycode[n++] = HID_KEY_ARROW_RIGHT;
    }

    if (pressed(PIN_TEST_CAL) && n < 6) {
        keycode[n++] = (PLAYER_ID == 1) ? HID_KEY_KEYPAD_7 : HID_KEY_KEYPAD_8;
    }

    if (pressed(PIN_COIN) && n < 6) {
        keycode[n++] = (PLAYER_ID == 1) ? HID_KEY_KEYPAD_5 : HID_KEY_KEYPAD_6;
    }

    if (pressed(PIN_START) && n < 6) {
        keycode[n++] = (PLAYER_ID == 1) ? HID_KEY_KEYPAD_1 : HID_KEY_KEYPAD_2;
    }

    if (active) {
        if (pressed(PIN_BOMB_SELECT) && n < 6) {
            keycode[n++] = (PLAYER_ID == 1) ? HID_KEY_SPACE : HID_KEY_RIGHT_CTRL;
        }

        if (pressed(PIN_RELOAD) && n < 6) {
            keycode[n++] = (PLAYER_ID == 1) ? HID_KEY_R : HID_KEY_T;
        }
    }
...
int main(void) {
    ...
    setup_pin(PIN_BOMB_SELECT);
    setup_pin(PIN_TRIGGER);
    setup_pin(PIN_RELOAD);
    setup_pin(PIN_COIN);
    setup_pin(PIN_START);
    setup_pin(PIN_TEST_CAL);
    ...
    while (true) {
        ...
        if (cal && !last_cal && tud_cdc_connected()) {
            ...
            snprintf(b, sizeof(b), "CALREQ,P%d\n", PLAYER_ID);
